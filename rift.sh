...
uci set uhttpd.podkop_panel.max_requests='20'
uci set uhttpd.podkop_panel.cgi_prefix='/cgi-bin'
uci commit uhttpd

# 4. Setup DNS (rift -> 192.168.47.1)
...
if method == "get_network" then
    ...
    local domain_list = {}
    local raw_domains = exec_read("uci -q get podkop.main.user_domains")
    for w in raw_domains:gmatch("%S+") do table.insert(domain_list, w) end
    print(to_json({clients = clients, vpn_ips = vpn_list, domains = domain_list}))
    os.exit(0)
end
...
if method == "manage_domain" then
    ...
    local action = params.action
    if domain and action and domain:match("^[a-zA-Z0-9%.%-]+$") then
        if action == "add" then
            exec_silent("uci set podkop.main.user_domain_list_type='dynamic'")
            exec_silent("uci add_list podkop.main.user_domains='" .. domain .. "'")
        elseif action == "del" then
            exec_silent("uci del_list podkop.main.user_domains='" .. domain .. "'")
        end
        exec_silent("uci commit podkop")
        exec_silent("/etc/init.d/podkop restart")
        print('{"status":"ok"}')
...
<head>
    ...
    <style>
        ...
        .vpn-switch { background: rgba(255,255,255,0.1); padding: 6px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; cursor: pointer; text-transform: uppercase; display: inline-block; }
        .vpn-switch.on { background: var(--accent-dim); color: var(--accent); border: 1px solid var(--accent); }
        .vpn-switch.off { color: var(--text-sec); border: 1px solid transparent;}
        .vpn-switch.off:hover { background: rgba(255,255,255,0.15); }
        .url-spoiler { margin-top: 15px; padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.05); text-align: center; }
        .btn-secondary { background: rgba(255,255,255,0.05); color: var(--text-sec); border: 1px solid rgba(255,255,255,0.1); padding: 8px 16px; border-radius: 8px; font-size: 12px; cursor: pointer; width: 100%; box-sizing: border-box; }
        ...
        .chip { background: rgba(255,255,255,0.1); padding: 6px 12px; border-radius: 16px; font-size: 13px; display: flex; align-items: center; gap: 8px; }
        .chip span { color: var(--danger); font-weight: bold; cursor: pointer; font-size: 16px; line-height: 1; }
...
<div class="container">
    ...
    <div class="card">
        ...
        <div class="url-spoiler">
            <button class="btn-secondary" onclick="toggleUrlInput()">Изменить ссылку подписки</button>
...
<script>
    ...
    async function loadData(){
        try{
            let d=await api('get_nodes'); globalNodes=d.nodes||[]; activeUrl=d.active_url||"";
            let et=d.expire?"Истекает: "+d.expire:"Нет данных о подписке";
            document.getElementById('sub_meta').innerText=et;
            let an="Нет подключения", ann=null;
            if(activeUrl){
                let n=globalNodes.find(x=>x.full_url.trim()===activeUrl.trim());
                if(n){an=n.name;ann=n.name}else{let m=activeUrl.match(/#(.*)$/);if(m){an=decodeURIComponent(m[1]);ann=an}}
            }
            document.getElementById('active_name').innerText=an;
            renderNodes(ann)
        }catch(e){}
    }
    function renderNodes(ann){
        let tb=document.querySelector("#nodes_table tbody");
        if(globalNodes.length===0){tb.innerHTML='<tr><td colspan="3" style="text-align:center; padding:10px;">Список пуст</td></tr>';return}
        let h="";
        globalNodes.forEach((n,i)=>{
            let ia=(n.full_url.trim()===activeUrl.trim())||(ann&&n.name===ann);
            let bc=ia?"vpn-switch on":"vpn-switch off", bt=ia?"ПОДКЛЮЧЕН":"ПОДКЛЮЧИТЬ", cl=ia?"":`onclick="connect(${i})"`, st=ia?"cursor:default":"";
            h+=`<tr><td><span class="srv-name">${n.name}</span></td><td style="text-align:right; width:60px;"><span id="ping_${i}" class="srv-ping">-</span></td><td style="text-align:right; width:110px;"><div class="${bc}" style="${st}" ${cl}>${bt}</div></td></tr>`
        });
        tb.innerHTML=h
    }
    ...
    async function toggleDom(dm,a){showLoader();try{await api('manage_domain',{domain:dm,action:a});await new Promise(r=>setTimeout(r,1500));await loadNetwork()}catch(e){alert("Ошибка")}finally{hideLoader()}}
    function addDomain(){let d=document.getElementById('new_domain').value;if(d)toggleDom(d,'add');document.getElementById('new_domain').value=""}
</script>
</body>
</html>
EOF
...
echo "Установка завершена! Панель доступна по адресу: http://rift:2017"
