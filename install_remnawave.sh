...
install_packages() {
    ...
    # UFW
    if ! ufw allow 22/tcp comment 'SSH' || ! ufw allow 10443/tcp comment 'HTTPS' || ! ufw --force enable; then
...
services:
  ...
  remnawave:
    ...
    ports:
      ...
      - '127.0.0.1:3002:\${METRICS_PORT:-3002}'
    networks:
      - remnawave-network
    healthcheck:
      test: ['CMD-SHELL', 'curl -f http://localhost:\${METRICS_PORT:-3002}/health']
      interval: 30s
      timeout: 5s
      retries: 3
      start_period: 30s
